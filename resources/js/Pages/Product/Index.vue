    ...
    <div class="grid grid-cols-6 gap-4">
      ...
      <nav class="text-black font-bold mt-3 col-start-1 col-end-7" aria-label="Breadcrumb">
        <ol class="list-none p-0 inline-flex">
          ...
          <li>
            <a class="text-gray-500" aria-current="page">Продукты</a>
          </li>
        </ol>
      </nav>
                ...
                <tbody v-if="products.length">                   
                    <tr v-for="product in products" :key="product.id">
                          ...
                          <div class="flex items-center">
                              <div class="ml-3">
                                  <p class="text-gray-900 whitespace-no-wrap">
                                      <inertia-link :href="route('subproducts',  [bot.id, product.id])">
                                        <button class="px-8 py-2 border text-blue-600 hover:shadow-md">{{ product.name }}</button>
                                      </inertia-link> 
                                  </p>
                              </div>
                          </div>
                      ...
                      <td class="px-5 py-3 border-b border-gray-200 bg-white text-sm">
                        ...
                        <button type="button" @click="deleteCategory(product.id)" class="text-sm bg-red-500 hover:bg-red-700 text-white py-1 px-2 rounded focus:outline-none focus:shadow-outline"><icon name="delete" class="w-4 h-4"/></button>
                      </td>
                    </tr>                                               
                </tbody>
